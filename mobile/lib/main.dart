...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    rtcManager.onIncomingCall = (call) {
      if (!mounted) return;

      final callerId = call['from_id']?.toString() ?? '';
      final callerName =
...
class _IncomingCallScreenState extends State<IncomingCallScreen> {
  ...
  void initState() {
    ...
    _subscription = CallSession.instance.socket.messages.listen((message) {
      final type = message['type']?.toString();
      final fromId = message['from_id']?.toString();

      if ((type == 'hangup' || type == 'call_reject') &&
          (fromId == null || fromId == widget.id)) {
...
class _CallScreenState extends State<CallScreen> {
  ...
  void initState() {
    ...
    _subscription = manager.session.socket.messages.listen((message) {
      final type = message['type']?.toString();
